...
class Party extends EventEmitter {
	...
	installHooks() {
		...
		this.installHook('S_PARTY_MEMBER_LIST', 7, event => {
			...
			this.emit('party_list', this.partyMembers);
		});

		this.installHook('S_LEAVE_PARTY_MEMBER', 2, event => {
			this._generateSimpleEvent(event, "party_member_leave")
		});

		this.installHook('S_BAN_PARTY_MEMBER', 1, event => {
			this._generateSimpleEvent(event, "party_member_kick")
		});
		...
		this.installHook('S_SPAWN_USER', 15 , event => {
			...
			if(wasBugFixed) this.emit('party_list', this.partyMembers);
		});
	}
