...
class Contract extends EventEmitter {
    ...
    installHooks() {
        ...
        this.installHook('S_REQUEST_CONTRACT', 1, (event) => {
            this.id = event.id;
        ...
        this.installHook('S_ACCEPT_CONTRACT', 'raw', (event) => {
            this.reset();
            this.emit('end', 'accept');
        });

        this.installHook('S_REJECT_CONTRACT', 'raw', (event) => {
            this.reset();
            this.emit('end', 'reject');
        });

        this.installHook('S_CANCEL_CONTRACT', 'raw', (event) => {
            this.reset();
