...
class Inventory extends EventEmitter
{
    ...
    installHooks()
    {
        ...
        this.installHook('S_INVEN', this.parent.mod.majorPatchVersion >= 80 ? 18 : 17, (event) => {
            ...
            if (!event.more) {
                ...
                this._buffer.items.forEach(item => {
                    // Convert single passivity set from classic to modern multi-option set format
                    // Fill fields that didn't exist in classic with reasonable default values
                    if (this.parent.mod.isClassic) {
                        item.awakened = false;
                        item.liberationStatus = 0;
                        item.feedstock = 0;
                        item.boundToItem = 0;
                        item.hasEtching = false;
                        item.pcbang = false;
                        item.xp = 0n;
                        item.damaged = false;

                        item.passivitySet = 0;
                        item.extraPassivitySets = 0;
                        item.passivitySets = [{
                            index: 0,
                            masterworkBonus: item.masterworkBonus,
                            itemLevel: item.itemLevel,
                            minItemLevel: 0,
                            maxItemLevel: 0,
                            passivities: item.passivities
                        }];

                        delete item.masterworkBonus;
                        delete item.itemLevel;
                        delete item.passivities;
                    }

                    if (this.parent.mod.majorPatchVersion < 80)
                        item.mergedPassivities = [];
